...
function simulate_observed_scores(parms::Parameters, results::DataFrame, ability_dist::Distribution=Normal(0.0, 1.0))
    bank = parms.bank
...
function generate_characteristic_curves(parms::Parameters, results::DataFrame, theta_range::AbstractVector, r::Int=1)
    bank = parms.bank
...
function generate_information_curves(parms::Parameters, results::DataFrame, theta_range::AbstractVector)
    bank = parms.bank
    num_forms = size(results, 2)
    curves = DataFrame()
...
function plot_results(parms::Parameters, conf::Config, results::DataFrame, theta_range::AbstractVector=-3.0:0.1:3.0, plot_file::String="results/combined_plot.pdf")
    # Generate plot data
    ...
    if parms.method != "TIC3"
        ...
        simulation_data1 = simulate_observed_scores(parms, DataFrame(data=results[:, 1]), Normal(-1.0, 0.8))
        simulation_data2 = simulate_observed_scores(parms, DataFrame(data=results[:, 2]), Normal(0.0, 1.0))
        simulation_data3 = simulate_observed_scores(parms, DataFrame(data=results[:, 3]), Normal(1.0, 0.7))
    end
    ...
    p1 = @df characteristic_curves plot(theta_range, cols(), title="Characteristic Curves",
        xlabel="θ", ylabel="Score", linewidth=2, label="", grid=(:on, :lightgray, :solid, 1, 0.9), tickfontsize=12, titlefontsize=16)
    ...
    p2 = @df information_curves plot(theta_range, cols(), title="Information Curves",
        xlabel="θ", ylabel="Information", linewidth=2, label="", grid=(:on, :lightgray, :solid, 1, 0.9), tickfontsize=12, titlefontsize=16)

    # Only one simulation chart with all variations
    p3 = @df simulation_data1 plot(1:size(simulation_data1, 1), cols(), title="Observed Scores (Simulations)",
        xlabel="Items", ylabel="Percentage", linewidth=2, label="", grid=(:on, :lightgray, :solid, 1, 0.9), tickfontsize=12, titlefontsize=16)

    # Add the other simulations to the same plot
    p3 = @df simulation_data2 plot!(1:size(simulation_data2, 1), cols(), linewidth=2, label="")
    p3 = @df simulation_data3 plot!(1:size(simulation_data3, 1), cols(), linewidth=2, label="")
